...
public class Client implements Runnable {
    ...
    private void onReceiveNextRound(String received) {
        String[] splitted = received.split(";");
        if (splitted.length >= 5 && splitted[1].equals("success")) {
    ...
    private void onReceiveSubmitResult(String received) throws SQLException {
        ...
        System.out.println("Received guess from " + user + " in room " + roomId + ": " + priceGuess);
        ...
        if (room != null && room.isGameStarted()) {
            ...
            if (room.getPriceGuessClient1() != 0 && room.getPriceGuessClient2() != 0) {
                System.out.println("Both players have submitted guesses. Ending round for room " + roomId);
                room.handleRoundEnd();
            }
        }
    }
